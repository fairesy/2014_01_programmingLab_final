...
void addNewMember(void)
{
	...
	gotoxy(18, 8); printf("메인메뉴로 돌아가시려면 0을 눌러주세요");
...
void editMember(int memberIdx)
{
	...
	{
	case '1'://이름
		gotoxy(0, 18); printf("\t\t    새로운 이름을 입력하세요 : ");
		...
		while (1)
		{
			...
			while (1)
			{
				if (buf[0] == '\n')
				{
					chk = 0;
					gotoxy(0, 19); printf("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t");
					gotoxy(0, 19); printf("\t\t    이름을 입력하세요 : "); fgets(buf, sizeof(buf), stdin);
				}
				else break;
			}
			...
			gotoxy(0, 19); printf("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t");
			gotoxy(0, 19); printf("\t\t    올바른 이름을 입력해주세요 :");
		}

		break;

	case '2'://주소
		gotoxy(0, 18); printf("\t\t    새로운 주소을 입력하세요 : ");
		while (1)
		{
			fgets(buf, sizeof(buf), stdin);
			if (buf[0] != '\n'&&strlen(buf) <= 80)
	...
	case '3'://전화번호
		...
		gotoxy(0, 18); printf("\t\t    새로운 전화번호를 입력하세요 : ");
		...
		while (1)
		{
			...
			gotoxy(0, 19); printf("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t");
			gotoxy(0, 19); printf("\t\t    전화번호를 입력해주세요 :");
		}
...
void searchMember(int fromwhere)
{
	...
	gotoxy(20, 9); printf("4 메인메뉴로 돌아가기\n");
	...
	case '2':
		gotoxy(20, 13); printf("찾으실 이름 : ");
		fgets(searchName, sizeof(searchName), stdin);
		searchName[strlen(searchName) - 1] = '\0'; 
		//fgets로 받을때 엔터키('\n')까지 입력받아서 '이름\n'형식으로 들어오기때문에 마지막을 널문자로 바꿔서 저장해준다

		while (1)
		{//이름에 아무것도 입력하지 않고 엔터만 쳤을 경우(받을때 마지막을 \0으로 바꿨으니까 체크할때는 \0으로 체크) 다시 입력받기
			if (searchName[0] == '\0') 
			{
	...
	case '3':
		...
		while (1)
		{
			...
			textcolor(3);
			gotoxy(0, 15); printf("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t");
			gotoxy(20, 15); printf("형식에 맞추어 입력해주세요 : ");
			fgets(searchPhone, sizeof(searchPhone), stdin);
	...
	textcolor(7);
	if (i == curMemberNum){
		gotoxy(29, 19); printf("그런 학생 없습니다");
		getch();
		showMenu();
	}
	else
	{
		...
		case 0: //메인메뉴-검색
			...
			break;
		case 1: //메인메뉴-수정
			gotoxy(20, 20); printf("계속하시려면 아무 키나 눌러주세요 :-)");
			getch();
			editMember(i);
			break;
		case 2: //메인메뉴-삭제
...
void searchSubmenu(int memberIdx)
{
	...
	textcolor(3);
	gotoxy(20, 19); printf("1 삭제하기  2 수정하기  3 메인메뉴로");
	textcolor(7);
	gotoxy(20, 21); printf("다음에 할 작업을 선택해주세요 : ");
