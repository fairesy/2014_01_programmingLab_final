...
void showMenu(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		gotoxy(32, 9 + i); puts(menu[i]); //메뉴목록 출력
	}

	textcolor(3);
	gotoxy(17, 18); printf("▨▧▨▧▨▧▨▧▨▧▨▧▨▧▨▧▨▧▨▧▨▧▨");
	gotoxy(47, 19); printf("2014_1_shinyoung");
	textcolor(7);

	gotoxy(24, 16); printf("원하는 작업을 선택해주세요  ");
	choice = getche();
...
void phoneFormCheck(void) //전화번호를 올바른 형식으로 입력하도록 한다 
{
	int i;
...
void deleteMember(int memberIdx)
{
	...
	gotoxy(22, 8); printf("정말로 삭제할거에요? (y/n) : ");
	user_choice = getche();
	if (user_choice == 'y')
	{
...
void editMember(int memberIdx)
{
	...
	case '3'://전화번호
		...
		//수정할 사항 : phoneFormCheck에서의 형식에 맞추어 입력해주세요 가 무조건 호출
...
void searchMember(void)
{
	...
	gotoxy(20, 14); printf("원하는 작업을 선택해주세요 : ");
	choice = getche();
	Beep(rand() % 200 + 300, 80);
